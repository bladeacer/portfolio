  ...
  function render(results) {
    ...
    results.forEach(function (r, i) {
      ...
      if (q && displayLower.indexOf(q) > -1) {
        ...
        displayLabel =
          displayLabel.substring(0, idx) +
          '<mark class="search-highlight">' +
          displayLabel.substring(idx, idx + q.length) +
          "</mark>" +
          displayLabel.substring(idx + q.length);
      }
      var snippetHtml = "";
      if (q && r.snippet) {
        ...
        if (sIdx > -1) {
          snippetHtml =
            r.snippet.substring(0, sIdx) +
            '<mark class="search-highlight">' +
            r.snippet.substring(sIdx, sIdx + q.length) +
            "</mark>" +
            r.snippet.substring(sIdx + q.length);
        }
      }
      ...
      li.innerHTML =
        ...
        "</a>" +
        (snippetHtml
          ? '<div class="search-snippet">' + snippetHtml + "</div>"
          : "");
          ...
          if (fieldRes.result) {
            fieldRes.result.forEach(function (id) {
              ...
              if (p && !seenUrls[p.url]) {
                ...
                var rank =
                  p.title && p.title.toLowerCase().indexOf(lower) > -1 ? 0 : 1;
                var preview = (p.content || "").substring(0, 120);
                results.push({
                  title: p.title,
                  url: p.url,
                  breadcrumb: bc,
                  snippet: extractSnippet(p.content || "", q),
                  preview: preview,
                  rank: rank,
                });
              }
