...
var CANDIDATE_SELECTOR =
  ".content-wrapper > p:not(.foot):not(.cta):not(.cta2):not(.cta3), .content-wrapper > h1, .content-wrapper > h2, .content-wrapper > h3, .content-wrapper > h4, .content-wrapper > ul > li, .content-wrapper > ol > li, .content-wrapper > blockquote, .content-wrapper > pre, .content-wrapper > table, .content-wrapper .markdown-content p, .content-wrapper .markdown-content h1, .content-wrapper .markdown-content h2, .content-wrapper .markdown-content h3, .content-wrapper .markdown-content h4, .content-wrapper .markdown-content li, .content-wrapper .markdown-content pre, .content-wrapper .markdown-content table, .content-wrapper .markdown-content blockquote, .content-wrapper .blog-card";
...
function jumpToElement(direction, command, count) {
  ...
  if (command === "top") {
    ...
    nextIndex = findNextHeadingIndex(
      currentIndex,
      direction,
      candidates,
      count,
    ...
    targetScrollTop =
      window.scrollY + targetRect.top + targetRect.height / 2 - viewportCenterY;
  } else {
...
function navigateToNearestLink() {
  ...
  targetLink =
    ...
    lastHighlightedElement.querySelector("a");
...
function yankCurrentElement() {
  ...
  if (count > 1) {
    ...
    if (idx !== -1) {
      ...
      var texts = cs.slice(idx, endIdx).map(function (e) {
...
function yankToHeading(direction) {
  ...
  var texts = cs.slice(start, end).map(function (e) {
    return e.textContent.trim();
  });
  yankToClipboard(texts.join("\n"), "y" + (direction === 1 ? "}" : "{"));
}
...
document.addEventListener(
  ...
  function (e) {
    ...
    if (e.key === "y" && !e.shiftKey) {
      yPending = true;
      yPendingTimer = setTimeout(function () {
        yPending = false;
      }, 1000);
      return;
    }
